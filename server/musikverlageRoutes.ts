...
function startMusikverlagRebuildJob(id: MusikverlagId): Promise<void> {
  ...
  const job = (async () => {
    ...
    const { rowCount } = await rebuildMusikverlagTableDb(id, excelPaths, { uploadMode: "replace" });
  ...
  r.get("/admin/musikverlage", bearerAuth, requireAdmin, async (_req: Request, res: Response) => {
    try {
      ...
      for (const row of MUSIKVERLAGE_CATALOG) {
        ...
        const building = st?.tableDbBuildStatus === "running";
        const counted = diskDb && !building ? countRowsInMusikverlagDb(id) : null;
        const hasTableDb = diskDb && !building;
        ...
        const tableDbBuildStatus: "running" | "ok" | "error" | null = building
          ? "running"
          : hasTableDb
            ? "ok"
            : st?.tableDbBuildStatus === "error"
              ? "error"
              : null;
        entries[id] = {
          ...
          xlsxFileCount: hasFilesOnDisk ? files.length : uploadList.length,
          xlsxFileNames: hasFilesOnDisk
            ? uploadList.length
              ? uploadList.map((x) => x.originalFileName)
              : files.map((p) => path.basename(p))
            : fileNames,
    ...
    (async () => {
      ...
      try {
        ...
        try {
          ...
          const { rowCount } = await rebuildMusikverlagTableDb(id as MusikverlagId, excelPaths, {
            uploadMode: mode,
          ...
          res.status(422).json({
            ...
            fileKept: true,
            hasFile: (await listUploadFiles(id as MusikverlagId)).length > 0,
            hasTableDb: musikverlagSqliteExists(id as MusikverlagId),
          });
          return;
        }
        res.json({
    ...
    if (!musikverlagSqliteExists("wcpm")) {
      if (tablePath) {
        try {
          ...
          await rebuildMusikverlagTableDb("wcpm", allPaths.length ? allPaths : tablePath);
        } catch (e) {
